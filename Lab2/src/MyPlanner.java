...
public class MyPlanner {
   ...
   public static MyState ucs(MyState start, MyState goal) throws Exception{ 
	   ...
	   Set<MyState> beenthere = new TreeSet<MyState>();
	   beenthere.add(start);
	   frontier.add(start);
	   int count = 0;
	   while(frontier.size() > 0){
		   ...
		   for(MyState a : actions){
			   ...
			   double acost;
			   System.out.println("child coordinates before " + child.x + " " + child.y);
			   child =transition(s,a);
			   System.out.println("child coordinates after " + child.x + " " + child.y);
			   acost = action_cost(a);
				   if(beenthere.contains(child)){
					   System.out.println("beenthere.contains");
					   oldchild = child;
